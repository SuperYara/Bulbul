...
async function fetchOpenAIResponse(userMessage) {
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: "gpt-3.5-turbo",
      messages: [{role: "system", content: "you are helpful english teacher"},
      {role: "system", content: "your name is abdullah"},
      {role: "system", content: "give informations about past simple"},
...
talkButton.onclick = async () => {
  if (peerConnection?.signalingState === 'stable' || peerConnection?.iceConnectionState === 'connected') {
    ...
    const userInput = document.getElementById('output').innerHTML;
